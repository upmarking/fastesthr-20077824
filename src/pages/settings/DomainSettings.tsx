          ...
          {editingSlug ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Button variant="default" size="icon" className="h-10 w-10 shrink-0" onClick={handleSaveSlug} disabled={!slugAvailable || updateSlug.isPending || checkingSlug} aria-label="Save domain slug">
                  {updateSlug.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}
                </Button>
                <Button variant="outline" size="icon" className="h-10 w-10 shrink-0" onClick={() => setEditingSlug(false)} aria-label="Cancel editing">
                  <X className="w-4 h-4" />
                </Button>
              </div>
